...
- The assessment experience may use the product-owner-accepted `prototype_questionnaire_v1` wording and option-order mapping for private integration. The missing historical codebook and unproven measurement equivalence remain explicit prototype limitations; neither may be described as independently verified. Public submission remains unavailable until the protected workflow and safety release gates pass.
...
- **RAG document disconnecting:** Patient-specific questionnaire tokens, feature vectors, token matrices, inference payloads, and session state must never be embedded, lexically indexed, or stored in scientific-document retrieval collections. General mental-health documents use physically or logically isolated vector and lexical/BM25 collections/namespaces with mandatory metadata such as `data_class=scientific_publication`, `document_scope=general_mental_health`, and `contains_patient_data=false`. Retrieval applies those filters before either search path and fails closed for missing or mismatched metadata.
...
No AI/RAG product-policy decision remains pending. The product owner accepted the current questionnaire option-order mapping for `prototype_questionnaire_v1` on 2026-09-24; historical source-codebook and measurement-equivalence evidence remain unresolved as documented in [`ML_QUESTIONNAIRE_COMPATIBILITY_AUDIT.md`](ML_QUESTIONNAIRE_COMPATIBILITY_AUDIT.md). The private ML adapter may be integrated, while public submission waits for protected workflow and safety release evidence. Access control, operating envelope, corpus scope, live PubMed fallback, BERTScore policy, public limits, budget exhaustion behavior, and emergency-resource reverification remain approved. Exact AI model revisions and calibrated thresholds remain measured implementation/release gates rather than permission to weaken Sections 1–11.
