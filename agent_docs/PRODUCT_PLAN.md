...
| AC-27 | After safety interception, the language gate admits supported English only. Unsupported or uncertain language receives the fixed English-only response and cannot reach intent classification, RAG, LLM generation, or DCMFNet. | AI Architect / AI Engineer / Testing Agent / Reviewer |
...
| Local/private-VPC LLM or embedding model is not selected. | Local reproducibility, cost, privacy, and tests remain uncertain. | AI Architect benchmarks only local or operator-controlled isolated India-fenced deployments; public inference APIs cannot receive runtime payloads. |
...
- Structured internal contracts and deterministic mocks/fakes are required for tests involving local/private-VPC LLM or embedding adapters and external bibliographic services; tests do not send runtime-style payloads outside the process.
...
6. **Provider constraints:** Which English-capable local or operator-controlled isolated VPC LLM and embedding configurations balance reproducibility, cost, latency, and portfolio usability within the no-external-payload boundary?
7. **Resolved — privacy and lifecycle:** Zero persistent sensitive storage; exact 15-minute inactivity expiry; explicit reset; client/server volatile-memory wiping; no public provider payloads; allowlisted telemetry; aggregate-only unlinkable analytics; and no upload surface.
