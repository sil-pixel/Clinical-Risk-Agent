...
8. **AI-01 — Implement intent and LangGraph workflow** (AI Engineer): build the post-safety English-language gate, explicit state, routing, questionnaire progression, inference/retrieval tool calls, and failure branches using the approved AI architecture and stable ML/RAG contracts. Fine-tune and calibrate `distilbert-base-uncased` on the English routing dataset, pin the artifact, and implement deterministic abstention. Depends on AIARCH-01, ML-02, and RAG-01. Done when AC-02 through AC-05 and AC-27/AC-28 pass focused tests.
...
10. **BE-01 — Expose and deploy application APIs** (Backend Engineer): implement FastAPI validation, service wiring, volatile session/state integration, validated SSE, errors, readiness, and the Modal deployment adapter using established contracts. Depends on AI-02. Done when AC-11, AC-32, and backend portions of AC-34/AC-35 pass integration tests.

### M4 — User experience

11. **FE-01 — Implement core Framer journeys** (Frontend Engineer): build code components for questionnaire, conversation, result, evidence, validated streaming, and limitation experiences against the stable API. Depends on BE-01. Done when AC-01, AC-12, AC-31, and AC-32 pass with real service integration.
12. **FE-02 — Complete accessible failure, offline, and recovery states** (Frontend Engineer): cover missing data, validation errors, no evidence, service failures, offline state, and expired/missing assessment state. Depends on FE-01. Done when AC-33 passes and the UI never substitutes a plausible-looking result for failure.
