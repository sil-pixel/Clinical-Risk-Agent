...
| The scientific-source policy is approved, but no compliant corpus or retrieval adapter is implemented yet. | RAG cannot meet citation and grounding criteria until eligibility, quality, reranking, conflict, update, and retraction controls are verified. | RAG Engineer implements and validates the approved policy, including fortnightly incremental ingestion and independent weekly retraction monitoring; AI Architect reviews any required design change. |
...
4. **Resolved — scientific-source policy:** Admit only eligible DOI/PMID-bearing peer-reviewed, PubMed-indexed, authoritative-health-organization, or clinical-guideline publications from the rolling prior 20 years. Exclude preprints, theses, curated local PDFs, general websites, retracted sources, and low-quality studies. Apply the approved evidence hierarchy plus recency/quality metadata reranking, state unresolved controversy without choosing a side, ingest incrementally every two weeks, monitor retractions weekly, and cite only evidence retrieved for the current answer. Concrete licensing checks, quality instruments, and adapters remain implementation selections subject to this policy.
