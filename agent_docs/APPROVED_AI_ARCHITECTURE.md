...
6. `load_questionnaire_requirements` loads a versioned ML contract or returns `questionnaire_contract_unavailable`. The product owner accepted the current option order as a prototype mapping on 2026-09-24; `prototype_questionnaire_v1` is implemented for private local validation and assembly. Independent historical source-codebook and measurement equivalence remain unresolved, and public submission remains unavailable until protected workflow and safety release gates pass.
7. `validate_questionnaire` validates all visible values against the accepted versioned prototype contract. It cannot impute, infer, or transform answers outside that explicit mapping.
...
2. Integrate the product-owner-accepted `prototype_questionnaire_v1` private adapter documented in [`ML_ENGINEER_HANDOFF.md`](ML_ENGINEER_HANDOFF.md), with complete structured answers, artifact/version checks, deterministic safety and route authorization, safe result validation, and protected API/session handling. Historical codebook and measurement equivalence remain unverified prototype limitations, not evidence of clinical validity. `questionnaire_contract_unavailable` still applies to missing or unsupported contract versions.
